    ...
    <section id="home" className="relative h-screen flex items-center justify-center overflow-hidden bg-brand">
      <div className="absolute inset-0 opacity-30">
        <Canvas camera={{ position: [0, 0, 5], fov: 60 }}>
          ...
          <pointLight position={[5, 5, 5]} intensity={1.2} color="#fffbeb" />
          <FloatingShape />
        </Canvas>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}
        ...
        <p className="text-amber uppercase tracking-[0.4em] text-sm mb-4">Railway Station Road, Khammam</p>
        ...
        <div className="flex gap-4 justify-center flex-wrap">
          <a href="#rooms" className="bg-amber text-brand px-8 py-3 rounded-full font-semibold hover:bg-brand/5 transition-all hover:scale-105">
            Explore Rooms
          </a>
          <a href="#booking" className="border-2 border-amber text-amber px-8 py-3 rounded-full font-semibold hover:bg-amber hover:text-brand transition-all">
            Book Now
          </a>
        </div>
        <div className="mt-12 flex justify-center gap-8 text-cream/70 text-sm">
          <span>★ 4.0 / 5 on Google</span>
          <span>|</span>
