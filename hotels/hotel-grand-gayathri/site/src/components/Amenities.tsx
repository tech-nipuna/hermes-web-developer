    ...
    <section id="amenities" className="py-24 bg-cream">
      <div className="max-w-6xl mx-auto px-6">
        <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-16">
          <h2 className="text-5xl font-heading text-maroon mb-4">Amenities</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {amenities.map((a, i) => (
            <motion.div key={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
              ...
              <div className="w-14 h-14 rounded-xl bg-gold/10 flex items-center justify-center mx-auto mb-4 group-hover:bg-gold group-hover:text-white transition-all">
                <a.icon size={28} className="text-gold group-hover:text-white" />
              </div>
              <h3 className="font-semibold text-maroon mb-1">{a.label}</h3>
