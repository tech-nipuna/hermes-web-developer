    ...
    <section id="reviews" className="py-24 bg-white">
      <div className="max-w-4xl mx-auto px-6 text-center">
        <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
          className="text-5xl font-heading text-maroon mb-4">Guest Reviews</motion.h2>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={idx} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -40 }} className="bg-cream rounded-3xl p-10 shadow-sm">
            <Quote size={36} className="text-gold mx-auto mb-4" />
            ...
            <p className="font-semibold text-maroon">{reviews[idx].name}</p>
            <p className="text-gray-400 text-sm">{reviews[idx].date}</p>
          </motion.div>
        </AnimatePresence>
        <div className="flex justify-center gap-4 mt-8">
          <button onClick={prev} className="w-12 h-12 rounded-full bg-maroon text-ivory flex items-center justify-center hover:bg-gold hover:text-maroon transition-colors">
            <ChevronLeft size={20} />
          </button>
          <button onClick={next} className="w-12 h-12 rounded-full bg-maroon text-ivory flex items-center justify-center hover:bg-gold hover:text-maroon transition-colors">
