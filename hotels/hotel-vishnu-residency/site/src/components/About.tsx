...
export default function About() {
  const stats = [
    { num: '3.8', label: 'Google Rating', desc: 'Based on 928 guest reviews' },
    { num: 'rs.1,000', label: 'Starting Rate', desc: 'Per night, excluding taxes' },
    { num: '928', label: 'Happy Guests', desc: 'Reviewed on Google Maps' },
  ]
  return (
      ...
      <div className="max-w-6xl mx-auto px-6">
        <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-16">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Nestled on prestigious Balaji Nagar, Khammam, Hotel Vishnu Residency stands as a beacon of luxury in the heart of Khammam, Telangana.
          </p>
        </motion.div>
        <div className="grid md:grid-cols-3 gap-8">
          {stats.map((s, i) => (
            <motion.div key={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
              ...
              <p className="text-4xl font-bold text-accent">{s.num}</p>
